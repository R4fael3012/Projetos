...
SELECT SUM(seconds) AS soma_segundos_aulas FROM lessons WHERE course_id = 2;

SELECT course_id, COUNT(*) AS total_aulas FROM lessons GROUP BY course_id;

SELECT course_id, SUM(seconds) AS soma_segundos_aulas_cursos FROM lessons GROUP BY course_id;

SELECT course_id, SUM(seconds) FROM lessons GROUP BY course_id 
HAVING SUM(seconds) > 2300;

SELECT c."title" AS "curso", SUM(l."seconds") AS "total_seconds" FROM "lessons" l JOIN "courses" c ON l."course_id" = c."id" GROUP BY c."title" ORDER BY "total_seconds" DESC;

SELECT c."id" FROM "certificates" c JOIN "users" u ON c."user_id" = u."id" WHERE u."email" = 'mariana@email.com';
